...
async function loadBook(){
 ...
 content.innerHTML=`<div class="book-detail"><div><img class="book-cover-large" src="${esc(c)}" alt="${esc(b.title)}"></div><div class="book-info-large"><h1>${esc(b.title)}</h1><div class="book-author">نویسنده: ${esc(b.author||"نامشخص")}</div><div class="book-meta">${b.rating!=null?`<span>امتیاز ${esc(b.rating)}</span>`:""}</div><div class="book-description">${esc(b.description||"توضیحی ثبت نشده است.")}</div>${b.pdf_url?`<a class="read-button" href="${esc(b.pdf_url)}" target="_blank" rel="noopener">مطالعه کتاب</a>`:`<span>فایل PDF هنوز اضافه نشده است.</span>`}</div></div>`;
}

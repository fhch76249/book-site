...
async function saveBook(e){
 ...
 try{
  ...
  const payload={title,author:$("author").value.trim(),category:$("category").value.trim(),description:$("description").value.trim(),rating:$("rating").value.trim()===""?null:Number($("rating").value)};
...
async function saveAd(e){e.preventDefault();const p={title:$("adTitle").value.trim(),content:$("adContent").value.trim(),image_url:$("adImage").value.trim()||null,link_url:$("adLink").value.trim()||null,active:$("adActive").value==="true",start_at:toISO($("adStart").value),end_at:toISO($("adEnd").value)};if(!p.title){showMsg("عنوان تبلیغ را وارد کنید.",true);return}const id=$("adId").value.trim();const r=id?await db.from("ads").update(p).eq("id",id):await db.from("ads").insert(p);if(r.error)showMsg(r.error.message,true);else{showMsg(id?"تبلیغ ویرایش شد.":"تبلیغ اضافه شد.");resetAd();loadAds();loadSiteContent()}}
async function deleteAd(id){if(!confirm("تبلیغ حذف شود؟"))return;const {error}=await db.from("ads").delete().eq("id",id);if(error)showMsg(error.message,true);else{showMsg("تبلیغ حذف شد.");loadAds();loadSiteContent()}}
async function loadSiteContent(){
 const el=$("contentList");
 const {data,error}=await db.from("site_content").select("*").order("created_at",{ascending:false});
 if(error){el.textContent="خطا: "+error.message;return}
 el.innerHTML=(data||[]).map(x=>`<div class="ad-item"><strong>${esc(x.title||"بدون عنوان")}</strong><div class="muted">${x.active?"نمایش داده می‌شود":"مخفی است"}</div>${x.text?`<p>${esc(x.text)}</p>`:""}${x.image_url?`<img src="${esc(x.image_url)}" style="max-width:180px;max-height:120px;border-radius:10px;object-fit:cover" alt="">`:""}<div class="row-actions"><button class="primary" onclick="editSiteContent('${x.id}')">ویرایش</button><button class="danger" onclick="deleteSiteContent('${x.id}')">حذف</button></div></div>`).join("")||"<p>هنوز محتوایی ثبت نشده است.";
}
async function editSiteContent(id){const {data:x,error}=await db.from("site_content").select("*").eq("id",id).single();if(error){showMsg(error.message,true);return}$("contentId").value=x.id;$("contentTitle").value=x.title||"";$("contentText").value=x.text||"";$("contentImage").value=x.image_url||"";$("contentActive").value=String(x.active);$("contentSave").textContent="ذخیره تغییرات";$("contentCancel").style.display="inline-block";}
function resetContent(){$("contentForm").reset();$("contentId").value="";$("contentSave").textContent="ذخیره محتوا";$("contentCancel").style.display="none"}
async function saveSiteContent(e){e.preventDefault();const btn=$("contentSave");btn.disabled=true;try{let imageUrl=$("contentImage").value.trim()||null;const imageFile=$("contentImageFile")?.files?.[0];if(imageFile) imageUrl=await uploadFile(imageFile,"covers");const p={title:$("contentTitle").value.trim()||null,text:$("contentText").value.trim()||null,image_url:imageUrl,active:$("contentActive").value==="true"};if(!p.title&&!p.text&&!p.image_url)throw new Error("حداقل عنوان، متن یا عکس را وارد کنید.");const id=$("contentId").value.trim();const r=id?await db.from("site_content").update(p).eq("id",id):await db.from("site_content").insert(p);if(r.error)throw new Error(r.error.message);showMsg(id?"محتوا ویرایش شد.":"محتوا اضافه شد.");resetContent();loadSiteContent()}catch(err){showMsg(err.message||"خطا در ذخیره محتوا",true)}finally{btn.disabled=false}}
async function deleteSiteContent(id){if(!confirm("این محتوا حذف شود؟"))return;const {error}=await db.from("site_content").delete().eq("id",id);if(error)showMsg(error.message,true);else{showMsg("محتوا حذف شد.");loadSiteContent()}}

$("bookForm").addEventListener("submit",saveBook);$("cancelEdit").addEventListener("click",resetForm);$("adForm").addEventListener("submit",saveAd);$("contentForm").addEventListener("submit",saveSiteContent);$("contentCancel").addEventListener("click",resetContent);$("adCancel").addEventListener("click",resetAd);$("logoutBtn").addEventListener("click",async()=>{await db.auth.signOut();location.href="login.html"});
requireAdmin().then(u=>{if(u){loadAdminBooks();loadViewStats();loadAds();loadSiteContent()}});

...
async function saveSiteContent(e){e.preventDefault();const p={title:$("contentTitle").value.trim()||null,text:$("contentText").value.trim()||null,image_url:$("contentImage").value.trim()||null,active:$("contentActive").value==="true"};if(!p.title&&!p.text&&!p.image_url){showMsg("حداقل عنوان، متن یا عکس را وارد کنید.",true);return}const id=$("contentId").value.trim();const r=id?await db.from("site_content").update(p).eq("id",id):await db.from("site_content").insert(p);if(r.error)showMsg(r.error.message,true);else{showMsg(id?"محتوا ویرایش شد.":"محتوا اضافه شد.");resetContent();loadSiteContent()}}
